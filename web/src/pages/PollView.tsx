...
import { ThumbsUp, Send, Trophy } from "lucide-react";
...
function hasVoted(pollId: string) { try { return !!(JSON.parse(localStorage.getItem(VOTED_KEY) || "{}")[pollId]); } catch { return false; } }
function markVoted(pollId: string) { try { const v = JSON.parse(localStorage.getItem(VOTED_KEY) || "{}"); v[pollId] = Date.now(); localStorage.setItem(VOTED_KEY, JSON.stringify(v)); } catch {} }
...
export default function PollView() {
    ...
    <div className="min-h-screen bg-warm-bg flex items-center justify-center px-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        <div className="text-center mb-6">
          <span className="font-playfair text-2xl font-bold text-charcoal"><span className="text-terracotta">Omni</span>Poll</span>
        </div>
        <div className="bg-warm-white rounded-2xl border border-clay/30 p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-slate text-sm mt-1">Code: <code className="font-mono text-terracotta">{code}</code></p>
          </div>
          {error ? <div className="bg-red-50 text-red-600 rounded-xl p-3 text-sm mb-4">{error}</div> : null}
          <div className="space-y-3">
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-charcoal">Your Name (optional)</label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Anonymous" onKeyDown={(e) => e.key === "Enter" && join()} className="bg-warm-white border-clay/40 focus:border-terracotta" />
            </div>
  ...
  if (!poll) return <div className="min-h-screen bg-warm-bg flex items-center justify-center"><div className="animate-spin w-8 h-8 border-2 border-terracotta border-t-transparent rounded-full" /></div>;

  return (
    <div className="min-h-screen bg-warm-bg px-4 py-10">
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          <span className="font-playfair text-xl font-bold text-charcoal"><span className="text-terracotta">Omni</span>Poll</span>
          <div className="text-xs text-slate font-mono mt-0.5">{code}</div>
        </div>

        {poll.type === "multiple_choice" && <MultipleChoice poll={poll} results={results} voted={voted} pid={pid} name={name} onVoted={() => { setVoted(true); markVoted(poll.id); }} onUpdate={(p, r) => { setPoll(p); setResults(r); }} />}
        {poll.type === "word_cloud" && <WordCloud poll={poll} results={results} voted={voted} pid={pid} name={name} onVoted={() => { setVoted(true); markVoted(poll.id); }} onUpdate={(p, r) => { setPoll(p); setResults(r); }} />}
        {poll.type === "qa" && <QAView poll={poll} results={results} pid={pid} name={name} onUpdate={(p, r) => { setPoll(p); setResults(r); }} />}
        {poll.type === "quiz" && <QuizView poll={poll} results={results} pid={pid} name={name} onUpdate={(p, r) => { setPoll(p); setResults(r); }} />}
        {poll.type === "rating" && <RatingView poll={poll} results={results} voted={voted} pid={pid} name={name} onVoted={() => { setVoted(true); markVoted(poll.id); }} onUpdate={(p, r) => { setPoll(p); setResults(r); }} />}
      </div>
    </div>
  );
}

// ─── Poll type components ────────────────────────────────────

interface PollProps {
  poll: Poll; results: PollResults | null; voted?: boolean; pid: string; name: string;
  onVoted?: () => void; onUpdate: (p: Poll, r: PollResults) => void;
}

function ResultBar({ text, votes, pct, index }: { text: string; votes: number; pct: number; index: number }) {
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between text-sm"><span className="font-medium text-charcoal">{text}</span><span className="font-mono text-slate">{votes} ({pct}%)</span></div>
      <div className="h-5 bg-parchment rounded-full overflow-hidden">
        <motion.div className="h-full bg-terracotta rounded-full" animate={{ width: `${pct}%` }} transition={{ duration: 0.6, delay: index * 0.05 }} />
      </div>
    </div>
  );
}
...
function ThankYou({ children }: { children?: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 mb-6 bg-sage/10 rounded-2xl p-4">
      ...
      <div><h3 className="font-semibold text-charcoal">Thank you!</h3><p className="text-sm text-slate">{children || "Your response was recorded."}</p></div>
    </div>
  );
}

function MultipleChoice({ poll, results, voted, pid, name, onVoted, onUpdate }: PollProps) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const data = await vote(poll.id, { participantId: pid, participantName: name || "Anonymous", answer });
      onVoted?.();
    ...
    <div className="bg-warm-white rounded-2xl border border-clay/30 p-6">
      ...
      {!voted && (
        <div className="space-y-3 mb-6">
          {poll.options.map(opt => (
            ...
            }} className={`w-full text-left px-4 py-3 rounded-xl border-2 font-medium text-sm transition-all ${selected.has(opt.id) ? "border-terracotta bg-terracotta/10 text-terracotta" : "border-clay/30 bg-parchment text-charcoal hover:border-terracotta/40"}`}>
              {opt.text}
            </button>
          ))}
          <Button onClick={submit} disabled={submitting || !selected.size} className="w-full bg-terracotta hover:bg-orange-600 text-white mt-2">
      ...
      {showResults && results?.options && (
        <div className="space-y-4 border-t border-clay/20 pt-4">
          ...
          {results.options.map((r, i) => <ResultBar key={r.id} text={r.text} votes={r.votes} pct={r.pct} index={i} />)}
        </div>
      )}
    </div>
  );
...
function WordCloud({ poll, results, voted, pid, name, onVoted, onUpdate }: PollProps) {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const data = await vote(poll.id, { participantId: pid, participantName: name || "Anonymous", answer: word.trim() });
  ...
  return (
    <div className="bg-warm-white rounded-2xl border border-clay/30 p-6">
      ...
      {voted ? <ThankYou>Your word was added to the cloud!</ThankYou> : (
        <div className="flex gap-2 mb-6">
          <Input value={word} onChange={(e) => setWord(e.target.value)} placeholder="Enter a word…" onKeyDown={(e) => e.key === "Enter" && submit()} className="flex-1 bg-warm-white border-clay/40 focus:border-terracotta" />
          <Button onClick={submit} disabled={submitting} className="bg-terracotta hover:bg-orange-600 text-white"><Send size={14} /></Button>
        </div>
      )}
...
function QAView({ poll, results, pid, name, onUpdate }: Omit<PollProps, "voted" | "onVoted">) {
  const [question, setQuestion] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [upvoted, setUpvoted] = useState<Set<string>>(() => {
    try { return new Set(JSON.parse(localStorage.getItem("omnipoll_upvoted_" + poll.id) || "[]")); }
    ...
    <div className="bg-warm-white rounded-2xl border border-clay/30 p-6">
      ...
      <div className="space-y-3 max-h-80 overflow-y-auto mb-5">
        {results?.questions?.length ? results.questions.map(q => (
          <div key={q.id} className="flex items-start gap-3 bg-parchment rounded-xl p-3">
            ...
            <button
              onClick={() => upvote(q)}
              className={`flex items-center gap-1 text-xs font-semibold px-2.5 py-1.5 rounded-lg transition-all whitespace-nowrap ${upvoted.has(q.id) ? "bg-terracotta text-white" : "bg-cream text-slate hover:bg-terracotta/10 hover:text-terracotta"}`}
            >
      ...
      <div className="border-t border-clay/20 pt-4">
        ...
        <div className="flex gap-2">
          <Input value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Type your question…" onKeyDown={(e) => e.key === "Enter" && submitQ()} className="flex-1 bg-warm-white border-clay/40 focus:border-terracotta" />
          <Button onClick={submitQ} disabled={submitting} className="bg-terracotta hover:bg-orange-600 text-white"><Send size={14} /></Button>
        </div>
      </div>
    </div>
  );
...
function QuizView({ poll, results, pid, name, onUpdate }: Omit<PollProps, "voted" | "onVoted">) {
  ...
  const handleAnswer = async (optId: string | null) => {
    ...
    try {
      const data = await submitQuizAnswer(poll.id, { participantId: pid, participantName: name || "Anonymous", questionId: qq.id, selectedAnswer: optId, timeLeft });
  ...
  if (!questions.length) return (
    <div className="bg-warm-white rounded-2xl border border-clay/30 p-6">
      <h2 className="font-playfair text-2xl font-bold text-charcoal">{poll.question}</h2>
      <p className="text-slate mt-2">No quiz questions configured.</p>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between text-sm text-slate">
        ...
        <div className="flex items-center gap-1 font-semibold text-terracotta"><Trophy size={14} /> {score}</div>
      </div>

      <div className="h-1.5 bg-parchment rounded-full">
        <motion.div className="h-full rounded-full" animate={{ width: `${pct}%` }} style={{ background: pct < 30 ? "#C94040" : pct < 60 ? "#D4A574" : "#D96C4A" }} transition={{ duration: 0.1 }} />
      </div>

      <div className="bg-warm-white rounded-2xl border border-clay/30 p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-playfair text-xl font-bold text-charcoal flex-1 pr-4">{qq.questionText || `Question ${qIndex + 1}`}</h2>
          <div className="text-2xl font-bold font-mono text-terracotta w-14 text-center">{timeLeft}s</div>
        </div>
        ...
        {!answered ? (
          <div className="space-y-3">
            {qq.options.map((opt, i) => (
              <button key={opt.id} onClick={() => handleAnswer(opt.id)}
                ...
                <span className="font-mono text-slate mr-2">{String.fromCharCode(65 + i)}.</span>{opt.text}
              </button>
            ))}
          </div>
        ) : (
          <div className="text-center space-y-4">
            ...
            {!isCorrect && <p className="text-sm text-slate">Correct answer: <span className="font-medium text-charcoal">{qq.options.find(o => o.id === qq.correctAnswer)?.text}</span></p>}
...
function RatingView({ poll, results, voted, pid, name, onVoted, onUpdate }: PollProps) {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const data = await vote(poll.id, { participantId: pid, participantName: name || "Anonymous", answer: selected });
      onVoted?.(); onUpdate(poll, data.results);
    } catch (e: unknown) { toast.error(e instanceof Error ? e.message : "Failed"); }
    finally { setSubmitting(false); }
  };
  ...
  return (
    <div className="bg-warm-white rounded-2xl border border-clay/30 p-6">
      ...
      {voted ? <ThankYou>Rating submitted!</ThankYou> : (
        <>
          ...
          <div className="flex flex-wrap gap-2 justify-center mb-6">
            {Array.from({ length: max - min + 1 }, (_, i) => min + i).map(v => (
              <button key={v} onClick={() => setSelected(v)}
                className={`w-12 h-12 rounded-xl border-2 font-bold text-sm transition-all ${selected === v ? "border-terracotta bg-terracotta text-white" : "border-clay/40 bg-parchment text-charcoal hover:border-terracotta/50"}`}>
                {v}
              </button>
            ))}
          </div>
          <Button onClick={submit} disabled={submitting || selected === null} className="w-full bg-terracotta hover:bg-orange-600 text-white">
