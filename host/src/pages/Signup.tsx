...
import {
  BarChart3, Eye, EyeOff, Loader2, Home, ArrowRight,
...
const PERKS = [
  ['All 20 poll types free', 'Unlimited classrooms'],
  ['Real-time results',      'Student key sheets'],
];
...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error((err as Error).message ?? 'Signup failed. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col font-body relative overflow-hidden"
      ...
      <div className="absolute inset-0 opacity-[0.035]"
        style={{ backgroundImage:'radial-gradient(circle, #D96C4A 1px, transparent 1px)', backgroundSize:'28px 28px' }}/>

      {/* Home button */}
      <div className="absolute top-4 left-4 z-20">
        <Link to="/" className="flex items-center gap-1.5 bg-white/80 backdrop-blur border border-cream-300 px-3 py-2 rounded-xl text-sm font-medium text-slate-600 hover:text-terracotta-600 hover:border-terracotta-300 shadow-sm transition-all">
          <Home size={14}/> Home
        </Link>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-16 relative z-10">
        ...
        <Link to="/" className="flex flex-col items-center gap-2 mb-6 group">
          <div className="w-14 h-14 bg-terracotta-500 rounded-2xl flex items-center justify-center shadow-lg group-hover:bg-terracotta-600 transition-colors">
            <BarChart3 size={26} className="text-white"/>
          </div>
          <span className="font-display font-bold text-2xl text-slate-800">OmniPoll</span>
        </Link>
        <p className="text-slate-500 text-sm mb-8">Create your free account — takes 30 seconds</p>

        <AnimatePresence mode="wait">
          {/* ─── STEP 1: Role picker (image 7) ─────────────────────────── */}
          {step === 1 && (
            <motion.div key="step1"
              ...
              className="w-full max-w-md bg-white/90 backdrop-blur border border-cream-300 rounded-2xl shadow-xl overflow-hidden"
            >
              <div className="p-7">
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {([
                    { r:'teacher' as const, icon: GraduationCap, label:'Teacher', sub:'Create & manage polls' },
                    { r:'student' as const, icon: BookOpen,       label:'Student', sub:'Join & take quizzes'  },
                  ]).map(opt => (
                    <motion.button key={opt.r}
                      ...
                      className={`relative flex flex-col items-center gap-3 p-5 rounded-2xl border-2 transition-all ${
                        role === opt.r
                          ...
                          : 'border-cream-300 hover:border-terracotta-200 bg-white'
                      }`}
                    >
                      {/* Check */}
                      {role === opt.r && (
                      ...
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm ${
                        role === opt.r ? 'bg-terracotta-500' : 'bg-cream-200'
                      }`}>
                        <opt.icon size={26} className={role === opt.r ? 'text-white' : 'text-slate-500'}/>
                      </div>
                      <div className="text-center">
                        <p className={`font-display font-bold ${role === opt.r ? 'text-terracotta-700' : 'text-slate-800'}`}>
                          {opt.label}
                        </p>
                ...
                <div className="bg-cream-50 border border-cream-200 rounded-xl p-3.5 mb-5">
                  <p className="text-xs font-bold text-slate-600 text-center mb-2.5">
              ...
              <div className="p-7">
                ...
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <div className="ml-auto flex items-center gap-1.5 bg-terracotta-50 border border-terracotta-200 px-2.5 py-1 rounded-full">
                    {role === 'teacher'
                      ? <GraduationCap size={12} className="text-terracotta-600"/>
                      : <BookOpen      size={12} className="text-terracotta-600"/>}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex gap-1">
                          {[1,2,3,4].map(n => (
                            <div key={n} className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                              strength >= n ? strengthColor : 'bg-cream-300'
                            }`}/>
                          ))}
                        </div>
                  ...
                  <motion.button type="submit" disabled={loading} whileTap={{ scale:0.98 }}
                    className="w-full flex items-center justify-center gap-2 bg-terracotta-500 hover:bg-terracotta-600 disabled:opacity-60 text-white py-3.5 rounded-xl font-bold transition-all shadow-md text-sm mt-2">
                    {loading
                  ...
                  <p className="text-center text-xs text-slate-400">
                    ...
                    <Link to="/contact" className="underline hover:text-terracotta-500">Terms</Link>
                    {' '}and{' '}
                    <Link to="/contact" className="underline hover:text-terracotta-500">Privacy Policy</Link>
                  </p>
                </form>
              </div>
        ...
        <div className="flex items-center gap-4 mt-6 text-xs text-slate-400">
          <Link to="/"       className="hover:text-terracotta-500 flex items-center gap-1 transition-colors"><Home size={11}/> Home</Link>
          <span>·</span>
