...
import {
  BarChart3, Cloud, HelpCircle, Trophy, Star, Zap,
  ArrowRight, Users, Globe, CheckCircle, Sparkles
} from 'lucide-react';
...
const POLL_TYPES = [
  { icon: BarChart3, label: 'Multiple Choice', desc: 'Real-time vote bars', color: 'bg-blue-50 text-blue-600 dark:bg-blue-950' },
  { icon: Cloud,     label: 'Word Cloud',      desc: 'Live word aggregation', color: 'bg-sky-50 text-sky-600 dark:bg-sky-950' },
  { icon: HelpCircle,label: 'Q&A Session',     desc: 'Upvote best questions', color: 'bg-amber-50 text-amber-600 dark:bg-amber-950' },
  { icon: Trophy,    label: 'Live Quiz',        desc: 'Timed scoring & leaderboard', color: 'bg-yellow-50 text-yellow-700 dark:bg-yellow-950' },
  { icon: Star,      label: 'Rating Scale',     desc: 'Customizable 1–10 scale', color: 'bg-orange-50 text-orange-600 dark:bg-orange-950' },
  { icon: Zap,       label: '15 More Types',    desc: 'NPS, Matrix, Heatmap…', color: 'bg-purple-50 text-purple-600 dark:bg-purple-950' },
];
...
const HOW = [
  { step: '1', title: 'Create', desc: 'Choose from 20 poll types and set up your question in seconds.' },
  { step: '2', title: 'Share', desc: 'Share a 6-char join code or link — no app download needed.' },
  { step: '3', title: 'Watch', desc: 'See results update live as participants vote in real-time.' },
];
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-warm-bg mesh-gradient">
      {/* ── Navbar ── */}
            ...
            {user ? (
              <>
                ...
                <Button size="sm" asChild>
                  <Link href="/create"><span className="text-sm">New Poll</span></Link>
                </Button>
              </>
            ) : (
              <>
      ...
      <section className="max-w-6xl mx-auto px-4 pt-20 pb-16 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <h1 className="font-playfair text-5xl md:text-7xl font-bold text-charcoal dark:text-foreground mb-6 leading-tight">
          ...
          <p className="text-slate dark:text-muted-foreground text-lg md:text-xl max-w-2xl mx-auto mb-10">
            Create polls, quizzes, word clouds, Q&A sessions and 16 more interactive types.
        ...
        >
          ...
          <Button onClick={handleJoin} disabled={joining} size="lg" className="gap-2">
            {joining ? 'Finding…' : 'Join Poll'} <ArrowRight className="w-4 h-4" />
          </Button>
        </motion.div>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-20">
        <div className="text-center mb-10">
          <h2 className="font-playfair text-3xl font-bold text-charcoal dark:text-foreground mb-2">
            Every type of engagement
          </h2>
          <p className="text-slate dark:text-muted-foreground">20 poll types built for every audience, topic, and moment</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {POLL_TYPES.map(({ icon: Icon, label, desc, color }, i) => (
            <motion.div
              key={label}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {HOW.map(({ step, title, desc }) => (
              <motion.div
                ...
                viewport={{ once: true }} transition={{ delay: Number(step) * 0.1 }}
                className="text-center"
              >
                <div className="w-12 h-12 rounded-full bg-terracotta/10 border-2 border-terracotta/30 text-terracotta font-playfair font-bold text-lg flex items-center justify-center mx-auto mb-4">
                  {step}
                </div>
      ...
      <section className="max-w-4xl mx-auto px-4 py-24 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <h2 className="font-playfair text-4xl font-bold text-charcoal dark:text-foreground mb-4">
            Start polling for free
          </h2>
          <p className="text-slate dark:text-muted-foreground mb-8 text-lg">
            No credit card. No download. Deploy to Vercel in 2 minutes.
          </p>
      ...
      <footer className="border-t border-clay/20 bg-warm-white/40 dark:bg-card/40 py-8">
        <div className="max-w-6xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs text-slate dark:text-muted-foreground">
            Built with Next.js 14 · Neon PostgreSQL · Pusher Channels · Vercel
          </p>
          <div className="flex gap-4 text-xs text-slate dark:text-muted-foreground">
            <Link href="/login" className="hover:text-terracotta transition-colors">Sign In</Link>
            <Link href="/signup" className="hover:text-terracotta transition-colors">Sign Up</Link>
            <Link href="/join" className="hover:text-terracotta transition-colors">Join Poll</Link>
          </div>
        </div>
      </footer>
    </div>
