...
import {
  BarChart2, Plus, Users, Activity, TrendingUp, Copy,
  ExternalLink, Play, Pause, X, MoreVertical, Clock, Trash2
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const fetchPolls = useCallback(async () => {
    if (!user?.id) return;
    try {
      const data = await getPolls(user.id) as { polls: Poll[] };
      setPolls(data.polls || []);
  ...
  useEffect(() => {
    ...
    if (user) fetchPolls();
  }, [user, authLoading, fetchPolls, router]);
  ...
  const STATS = [
    ...
    { icon: Users,       label: 'Participants',   value: totalParticipants, color: 'text-blue-600', bg: 'bg-blue-100 dark:bg-blue-900/30' },
    { icon: TrendingUp,  label: 'Responses',      value: totalResponses,  color: 'text-purple-600', bg: 'bg-purple-100 dark:bg-purple-900/30' },
  ];
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="sm:ml-auto">
            <Button asChild size="sm" className="gap-2">
              <Link href="/create"><Plus className="w-4 h-4" />New Poll</Link>
            </Button>
          </div>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-16 text-muted-foreground">
            ...
            <Button asChild size="sm"><Link href="/create">Create Poll</Link></Button>
          </div>
        ) : (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        ...
                        <Badge variant={poll.status === 'live' ? 'live' : poll.status === 'paused' ? 'paused' : 'closed'} className="flex-shrink-0">
                          {poll.status === 'live' && <span className="w-1.5 h-1.5 rounded-full bg-green-500 mr-1.5 animate-pulse" />}
                          {poll.status}
                        </Badge>
                      </div>
                  ...
                  <div className="flex items-center gap-2 mt-3 flex-wrap">
                    <Button variant="ghost" size="sm" className="gap-1.5 text-xs h-7"
                      onClick={() => copyJoinLink(poll.code)}>
                      <Copy className="w-3 h-3" />Copy link
                    </Button>
                    <Button variant="ghost" size="sm" className="gap-1.5 text-xs h-7" asChild>
                      <Link href={`/present/${poll.id}`}><ExternalLink className="w-3 h-3" />Present</Link>
                    </Button>
                    <Button variant="ghost" size="sm" className="gap-1.5 text-xs h-7" asChild>
                      <Link href={`/results/${poll.id}`}><BarChart2 className="w-3 h-3" />Results</Link>
                    </Button>
                    <div className="ml-auto flex items-center gap-1">
                      {poll.status === 'live' ? (
                        <Button variant="ghost" size="sm" className="gap-1 text-xs h-7 text-yellow-600"
                          onClick={() => setStatus(poll.id, 'paused')}>
                          <Pause className="w-3 h-3" />Pause
                        </Button>
                      ) : poll.status === 'paused' ? (
                        <Button variant="ghost" size="sm" className="gap-1 text-xs h-7 text-green-600"
